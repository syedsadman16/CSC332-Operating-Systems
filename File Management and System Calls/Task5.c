...
int main(int argc, char* argv[]){
    ...
    while((row = fgets(buffer, sizeof(buffer), text)) != NULL){
         ...
         * https://www.tutorialspoint.com/c_standard_library/c_function_sscanf.html
         * %n prints nothing and keeps track of the characters printed to an int
         */
    ...
    /*
    * After the 2D array is processed, create some Manager processes for each row
    * Manager will be assigned each students homework by number of rows
    */
    for(int i=0; i<studentRow; i++){
    	
    	manager = fork();
		if(manager < 0) {
			printf("Manager process could not be forked");
		}
		else if(manager == 0){
            /*
            * Each manager will create a worker per homework
            * The workers will calculate the averages and max scores
            */
